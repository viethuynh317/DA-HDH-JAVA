...
public class NonThread extends Thread{
    public int[][] graph;
    ...
    public NonThread(int[][] graph, int x, int y){
        this.x = x;
        this.y = y;
    ...
    public void solve(){
        ...
        for(int count = 0 ; count < this.graph.length -1; count ++){ 
           ...
           // System.out.println((newpick[0] + this.x + 1) + " " + (newpick[1] + this.x + 1) + "\t cost: " + this.graph[newpick[1]][newpick[0]]);
            NonThread.count();          
            for(int i = 0 ; i < this.graph.length; i ++){
